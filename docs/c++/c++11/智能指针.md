...
# 智能指针
## shared_ptr
shared_ptr实现原理是内部保存一个对象指针和一个引用计数类`_Ref_count`指针，通过`_Ref_count`继承自`_Ref_count_base`中引用计数变量来判断是否需要对保存的指针进行delete，当引用计数变量==0时，`_Ref_count`会对保存的对象，以及`_Ref_count`自身资进行释放
### 类`shared_ptr`分析
